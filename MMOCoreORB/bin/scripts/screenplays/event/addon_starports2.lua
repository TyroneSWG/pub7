...
function addon_starports2:teleportMB(pCollector, pPlayer)
	playerSwitchZoneOutOfCombat(pPlayer, "corellia", -148.3, 28, -4721.5, 85)
	return 0
end

function addon_starports2:teleportNB(pCollector, pPlayer)
	playerSwitchZoneOutOfCombat(pPlayer, "naboo", 5332.9, 10.9, 1367.9, 0)
	return 0
end

function addon_starports2:teleportAM(pCollector, pPlayer)
	playerSwitchZoneOutOfCombat(pPlayer, "rori", -5384.5, 75.7, -6013.3, 0)
	return 0
end

function addon_starports2:teleportSCE(pCollector, pPlayer)
	playerSwitchZoneOutOfCombat(pPlayer, "dungeon1", -0.1, 0.0, 145.6, 4336262)
	return 0
end

function addon_starports2:teleportRR(pCollector, pPlayer)
	playerSwitchZoneOutOfCombat(pPlayer, "rori", 5364.9, 80, 5664.1, -178)
	return 0
end

function addon_starports2:teleportTS(pCollector, pPlayer)
	playerSwitchZoneOutOfCombat(pPlayer, "naboo", -5042.8, -196.4, 6077.6, 40)
	return 0
end

function addon_starports2:teleportLK(pCollector, pPlayer)
	playerSwitchZoneOutOfCombat(pPlayer, "naboo", -6021.6, 19.5, 3021.5, 40)
	return 0
end

function addon_starports2:teleportED(pCollector, pPlayer)
	playerSwitchZoneOutOfCombat(pPlayer, "endor", -961.9, 73.0, 1546.8, -16)
	return 0
end

function addon_starports2:teleportDT(pCollector, pPlayer)
	playerSwitchZoneOutOfCombat(pPlayer, "talus", -3435.0, 55.3, 3375.0, 38)
	return 0
end

function addon_starports2:teleportecho(pCollector, pPlayer)
	playerSwitchZoneOutOfCombat(pPlayer, "talus", -4478.4, 7.6, -3473.4, -61)
	return 0
end

function addon_starports2:teleportYV(pCollector, pPlayer)
	playerSwitchZoneOutOfCombat(pPlayer, "dantooine", -7149.8, 0.1, -6074.8, -127)
	return 0
end

function addon_starports2:teleportCor(pCollector, pPlayer)--current
	playerSwitchZoneOutOfCombat(pPlayer, "dungeon1", -20, 0, 0, 4336197)
	return 0
end
...
function addon_starports2:teleportImpCor(pCollector, pPlayer)--rebel
	...
	if (playerfaction:isRebel() == true) then
		...
		playerSwitchZoneOutOfCombat(pPlayer, "dungeon1", -40, 0, 0, 4336599)
	else
...
function addon_starports2:teleportPvpCorImp(pCollector, pPlayer)--Imp Bridge
	...
	if (playerfaction:isImperial() == true) then
		local player = LuaSceneObject(pPlayer)
		playerSwitchZoneOutOfCombat(pPlayer, "dungeon1", 18.6, 0, 123.6, 4336594)
	else
		local playerm = LuaCreatureObject(pPlayer)
		playerm:sendSystemMessage("You are not authorized to use this terminal")
	end	
...
function addon_starports2:teleportPvpCorReb(pCollector, pPlayer)--Rebel Load in
	...
	if (playerfaction:isRebel() == true) then
		local player = LuaSceneObject(pPlayer)
		playerSwitchZoneOutOfCombat(pPlayer, "dungeon1", 0.2, -6.7, -51.8, 4336555)
	else
		local playerm = LuaCreatureObject(pPlayer)
		playerm:sendSystemMessage("You are not authorized to use this terminal")
	end	
...
function addon_starports2:teleportRebCor(pCollector, pPlayer)--rebel
	local player = LuaSceneObject(pPlayer)
	playerSwitchZoneOutOfCombat(pPlayer, "dungeon1", -40, 0, 0, 4336331)
	return 0
end
...
function addon_starports2:teleportRebCor2(pCollector, pPlayer)--rebel
	...
	if (playerfaction:isImperial() == true) then
		local player = LuaSceneObject(pPlayer)
		playerSwitchZoneOutOfCombat(pPlayer, "dungeon1", -40, 0, 0, 4336264)
	else
		local playerm = LuaCreatureObject(pPlayer)
		playerm:sendSystemMessage("You are not authorized to use this terminal")
	end	

...
bloodfin_gift_store_template = ConvoTemplate:new {
   initialScreen = "first_screen",
   templateType = "Lua",
   luaClassHandler = "bloodfin_gift_store_convo_handler",
   screens = {}
}

-- 5 Token Exchange Screen

bloodfin_gift_store_five_screen = ConvoScreen:new {
   id = "five_screen",
   leftDialog = "",
   customDialogText = "For 5 Bloodfin Coins you can get one of these decorations! Remember to have one extra spot in your inventory to make room for whatever item you choose",
...
bloodfin_gift_store_template:addScreen(bloodfin_gift_store_five_screen);

-- 10 Token Exchange Screen

bloodfin_gift_store_ten_screen = ConvoScreen:new {
   id = "ten_screen",
   leftDialog = "",
...
bloodfin_gift_store_template:addScreen(bloodfin_gift_store_ten_screen);

-- 10 Token Exchange Screen 2

bloodfin_gift_store_ten2_screen = ConvoScreen:new { -- Not used unless you need more than one screen
   id = "ten2_screen",
   leftDialog = "",
   customDialogText = "For 10 Bloodfin Coins you can get one of these decorations! Remember to have one extra spot in your inventory to make room for whatever item you choose",
   stopConversation = "false",
...
bloodfin_gift_store_template:addScreen(bloodfin_gift_store_ten2_screen);

-- 30 Token Exchange Screen

bloodfin_gift_store_thirty_screen = ConvoScreen:new { -- Start of function
...
bloodfin_gift_store_template:addScreen(bloodfin_gift_store_thirty_screen);

-- 50 Token Exchange Screen

bloodfin_gift_store_fifty_screen = ConvoScreen:new {
   id = "fifty_screen",
   leftDialog = "",
...
bloodfin_gift_store_template:addScreen(bloodfin_gift_store_fifty_screen);

-- Main Screen

bloodfin_gift_store_first_screen = ConvoScreen:new {
   id = "first_screen",
   leftDialog = "",
   customDialogText = "Welcome to the Bloodfin Gift Store. Here you can buy many items using the Bloodfin Coins that can be found on various mobs around the galaxy. Have a look and see what you want!!",
   stopConversation = "false",
   options = {
...
bloodfin_gift_store_template:addScreen(bloodfin_gift_store_first_screen);

-- Purchase Completion

bloodfin_gift_store_end_quest = ConvoScreen:new {
   id = "end",
   leftDialog = "",
...
bloodfin_gift_store_template:addScreen(bloodfin_gift_store_end_quest);

-- Deny Option Convo

bloodfin_gift_store_deny_quest = ConvoScreen:new {
   id = "deny_quest",
   leftDialog = "",
...
bloodfin_gift_store_template:addScreen(bloodfin_gift_store_deny_quest);

-- Insufficient funds (used for credit purchases. not used for Bloodfin Coins)

bloodfin_gift_store_insufficient_funds = ConvoScreen:new {
...
bloodfin_gift_store_template:addScreen(bloodfin_gift_store_insufficient_funds);

-- Insufficient Space (get more inventory room!)

bloodfin_gift_store_insufficient_space = ConvoScreen:new {
...
bloodfin_gift_store_template:addScreen(bloodfin_gift_store_insufficient_space);

-- Insufficent Bloodfin Coins. (need to loot more Bloodfin Coins)

bloodfin_gift_store_insufficient_item = ConvoScreen:new {
...
bloodfin_gift_store_template:addScreen(bloodfin_gift_store_insufficient_item);

-- Finalize template

addConversationTemplate("bloodfin_gift_store_template", bloodfin_gift_store_template);

...
bool PlayerObjectImplementation::hasBadge(const String& badgeName) {
  const Badge* badge = BadgeList::instance()->get(badgeName);
  if (badge == NULL) {
    error("Asked for invalid badge: " + badgeName);
    return false;
  }

  const int badgeIndex = badge->getIndex();
  return hasBadge(badgeIndex);
}
...
void PlayerObjectImplementation::doFieldFactionChange(int newStatus) {
	...
	if (curStatus == FactionStatus::OVERT || curStatus == FactionStatus::COVERT  || curStatus == FactionStatus::ONLEAVE)
		return;

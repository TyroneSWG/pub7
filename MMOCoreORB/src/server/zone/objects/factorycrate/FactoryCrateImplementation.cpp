...
void FactoryCrateImplementation::fillAttributeList(AttributeListMessage* alm, CreatureObject* object) {
	...
	if(prototype == NULL || !prototype->isTangibleObject()) {
		object->sendSystemMessage("Whoops, it appears you have a broken crate. Please contact a GM or CSR by typing /csCreateTicket so we can remove that crate and give you your items before it does anything bad to your character =) Thank you!");
		return;
	}

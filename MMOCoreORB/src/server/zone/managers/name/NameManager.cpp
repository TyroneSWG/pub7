...
int NameManager::validateGuildName(const String& name, int type) {
	...
	if (strspn(name.toCharArray(), "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz1234567890'- ") != name.length())
...
int NameManager::validateChatRoomName(const String& name) {
	...
	if (hash == STRING_HASHCODE("Galaxy") || hash == STRING_HASHCODE("GroupChat"))
		return NameManagerResult::DECLINED_RESERVED;		

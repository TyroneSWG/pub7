...
int DirectorManager::createEvent(lua_State* L) {
	...
	if (numberOfArguments != 4 && numberOfArguments != 5 && numberOfArguments != 6) {
	...
	if (numberOfArguments == 4) {
		obj = (SceneObject*)lua_touserdata(L, -1);
		key = lua_tostring(L, -2);
		play = lua_tostring(L, -3);
		mili = lua_tonumber(L, -4);
	} else {
		args = lua_tostring(L, -1);
		obj = (SceneObject*)lua_touserdata(L, -2);
		key = lua_tostring(L, -3);
		play = lua_tostring(L, -4);
		mili = lua_tonumber(L, -5);
	}

...
Account* AccountManager::validateAccountCredentials(LoginClient* client, const String& username, const String& password) {
	...
	if(account == NULL) {
		...
		if (isAutoRegistrationEnabled() && client != NULL) {
			...
			{
				client->sendErrorMessage("Login Error", "You have already registered an account.  Please go to www.bloodfin.net and inquire on the forums how to setup a secondary account, or contact a GM in game by creating a ticket.");
				return NULL;
			}
		} else {
			if(client != NULL)
...
bool AccountManager::isRegistered(LoginClient* client)
{
	StringBuffer query;
	query << "SELECT a.ip_address FROM account_log a WHERE a.ip_address = '" << client->getSession()->getAddress().getIPAddress() << "' LIMIT 1;";
